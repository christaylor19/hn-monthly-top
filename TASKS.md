...
- [ ] **Whole-year / whole-week period selection** — let the picker select an entire year (top stories across all 12 months) or a single week, not just a month. Add a period-granularity toggle (Week / Month / Year) alongside the existing month + year `<select>`s in `MonthPicker.jsx`. Drive the Algolia query off a date range (`created_at_i` numericFilter between start/end of the chosen period) instead of the current per-month window, and update the hash URL scheme to encode the granularity (e.g. `2026`, `2026-06`, `2026-W25`).
